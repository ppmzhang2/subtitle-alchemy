...
def align_idx(seq_lab: list[str], seq_prd: list[str]) -> list[int]:
...
def get_aligned(seq_lab: list[str], seq_prd: list[str]) -> list[int]:
    ...
    """
    _mat_match = {}
    idx_aligned = align_idx(seq_lab, seq_prd)
    for idx_lab, idx_prd in enumerate(idx_aligned):
        if idx_prd != IDX_DEFAULT:
            _mat_match[idx_lab] = idx_prd
    sseq_idx_lab_mis, sseq_idx_prd_mis = locate_mismatch(
        idx_aligned, len(seq_prd))
